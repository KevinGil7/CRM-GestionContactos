...
const CrearProveedor: React.FC = () => {
  const [form, setForm] = useState<CreateProveedor>({
    name: "",
    email: "",
    phone: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const proveedorToSend = { ...form };
      if (!proveedorToSend.contactoPrincipal) delete proveedorToSend.contactoPrincipal;
      
      const response = await createProveedor(proveedorToSend);
      
      if (response.success) {
        toast.success("Proveedor creada correctamente");
        navigate("/home/proveedores");
      } else {
        toast.error("Error al crear proveedor: " + response.error?.error);
      }
    } catch (error) {
      console.error("Error al crear proveedor:", error);
      toast.error("Ocurrió un error inesperado al crear la proveedor");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">Completa la información del proveedor</p>
              </div>
            </div>
          </div>
        </div>
