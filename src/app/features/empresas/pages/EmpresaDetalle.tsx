...
import { getEmpresaBy, updateEmpresa, deleteEmpresa } from "../services/Empresa.service";
...
const EmpresaDetalle: React.FC = () => {
  ...
  const [empresa, setEmpresa] = useState<EmpresaBy | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (empresaResponse.success && empresaResponse.data) {
          ...
          setClientePrincipalId(empresaResponse.data.clientePrincipalDto?.id || null);
        }
  ...
  if (!empresa) return <div className="p-4 text-red-600">No se encontró la empresa.</div>;

  const handleChange = (field: keyof EmpresaBy, value: string) => {
  ...
  const handleGuardar = async () => {
    ...
    try {
      const empresaUpdate: CreateEmpresa = {
        name: empresa.name,
        email: empresa.email,
        phone: empresa.phone,
      ...
      const response = await updateEmpresa(empresa.id, empresaUpdate);
      if (response.success) {
      ...
      toast.error("Ocurrió un error al guardar la empresa");
    }
  };
  ...
  const clientePrincipalActual = empresa.clientePrincipalDto;
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-4 sm:space-y-0">
        ...
        >
          <Link to="/home/empresas" className="flex items-center text-indigo-600 hover:underline">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-gray-500">
                El cliente principal será el contacto principal de esta empresa
              </p>
            </div>
          </div>
        </div>
      ...
      {showDeleteModal && (
        <ConfirmModal
          ...
          title="¿Estás seguro?"
          message="Esta acción eliminará la empresa permanentemente."
        />
      )}
    </div>
  );
