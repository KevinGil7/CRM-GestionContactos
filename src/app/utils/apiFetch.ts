const apiUrl = new URL((import.meta?.env?.VITE_API_URL as string) || 'http://localhost:3000');
...
export const API_BASE_URL = import.meta.env.DEV ? devApiBaseUrl : (import.meta.env.VITE_API_URL as string) || '/';
...
const apiFetch = async (endpoint: string, options: FetchOptions = {}) => {
	...
	try {
		const response = await fetch(`${(import.meta.env.VITE_API_URL as string)}${endpoint}`, config);
